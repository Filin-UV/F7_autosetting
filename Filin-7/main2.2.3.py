...
class MainScreen(Screen):
    ...
    def update_video(self, dt):
        ...
        if video_debug == 0:
            ...
            np.copyto(self.frame1, self.video_input1.read()[1])
            p3 = time.perf_counter()
            np.copyto(self.frame2, cv2.resize(self.video_input2.read()[1], (640, 480), interpolation = cv2.INTER_AREA))
            p4 = time.perf_counter()
        else:
            np.copyto(self.frame1, cv2.imread("res/vis_test.jpg"))
            np.copyto(self.frame2, cv2.imread("res/uv_test.jpg"))

        if self.current_mode ==  2:
            if record_video:
                self.frame1 = cv2.putText(self.frame1, 'Rec~', (30, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2, cv2.LINE_AA)
            buf1 = cv2.flip(self.frame1, 0)
            buf = buf1.tobytes()
            self.texture.blit_buffer(buf, colorfmt='bgr', bufferfmt='ubyte')
            self.canva.texture = self.texture
        elif self.current_mode == 1:
            #self.frame2 = cv2.resize(self.frame2, (640, 480), interpolation = cv2.INTER_AREA)
            if record_video:
                self.frame2 = cv2.putText(self.frame2, 'Rec~', (30, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2, cv2.LINE_AA)
            buf1 = cv2.flip(self.frame2, 0)
            buf = buf1.tobytes()
            self.texture.blit_buffer(buf, colorfmt='bgr', bufferfmt='ubyte')
            self.canva.texture = self.texture
...
#                 self.histo_l.opacity = 100
                ...
                ai_frame = np.stack(((spot_color[2]*ai_frame).astype(np.uint8), (spot_color[1]*ai_frame).astype(np.uint8), (spot_color[0]*ai_frame).astype(np.uint8)), axis = -1)
################ai_frame = cv2.merge([(spot_color[2]*ai_frame).astype(np.uint8), (spot_color[1]*ai_frame).astype(np.uint8), (spot_color[0]*ai_frame).astype(np.uint8)])
                self.frame3 = cv2.addWeighted(cv2.subtract(self.frame1, ai_frame), 0.6, ai_frame, 1.0, 0)
                self.frame3 = cv2.putText(self.frame3, 'A', (40, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 255), 18, cv2.LINE_AA)
                self.frame3 = cv2.putText(self.frame3, 'A', (40, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 3, cv2.LINE_AA)
                self.frame3 = cv2.putText(self.frame3, 'c:{}'.format(area_avg), (40, 470), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 255), 12, cv2.LINE_AA)
                self.frame3 = cv2.putText(self.frame3, 'c:{}'.format(area_avg), (40, 470), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2, cv2.LINE_AA)
                if len(contours) > 0:
                    for contour in contours:
                        x, y, w, h = cv2.boundingRect(contour)
                        cv2.rectangle(self.frame3, (x, y), (x + w, y + h), (0, 0, 255), 4)
            else:
                self.histo_l.opacity = 0
                self.frame2_thr = np.stack(((spot_color[2]*self.frame2_thr).astype(np.uint8), (spot_color[1]*self.frame2_thr).astype(np.uint8), (spot_color[0]*self.frame2_thr).astype(np.uint8)), axis = -1)
################self.frame2_thr = cv2.merge([(spot_color[2]*self.frame2_thr).astype(np.uint8), (spot_color[1]*self.frame2_thr).astype(np.uint8), (spot_color[0]*self.frame2_thr).astype(np.uint8)])
                self.frame3 = cv2.addWeighted(cv2.subtract(self.frame1, self.frame2_thr), 0.6, self.frame2_thr, 1.0, 0)
            #(B, G, R) = cv2.split(frame2[1])
            #zero_arr = np.zeros(B.shape, dtype = uint8)
            #frame2 = cv2.merge([R*0,G,B])
            self.canva.canvas.ask_update()
            if record_video:
                self.frame3 = cv2.putText(self.frame3, 'Rec~', (30, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2, cv2.LINE_AA)
            buf1 = cv2.flip(self.frame3, 0)
            buf = buf1.tobytes()
            self.texture.blit_buffer(buf, colorfmt='bgr', bufferfmt='ubyte')
            self.canva.texture = self.texture
            try:
        ...
        if record_video:
            if new_video == False:
                ...
                if path != None:
                    mode_str = 'VID'
                    name = mode_str + '-' + datetime.datetime.strftime(datetime.datetime.now(), '%d-%m-%y-%H-%M-%S')
                    pathavi = path + '/' + name + '.avi'
                    self.out = cv2.VideoWriter(pathavi, fourcc, 20.0, (640,480))
            if self.current_mode == 2:
                self.out.write(self.frame1)
            elif self.current_mode == 1:
                self.out.write(self.frame2)
            elif self.current_mode == 0:
                self.out.write(self.frame3)
        else:
            try:
                self.out.release()
            except:
        ...
        print('p1={} ,p2={}, p3={}, p4={}, p5={}'.format(p1,p2,p3,p4,p5))
...
#             self.shot_button.text = next(click_types)
    ...
    def shot_callback(self):
        global record_video, new_video
        global frame1, frame2
        frame1 = self.frame1
        frame2 = self.frame2
        self.preview()
        print("Short clicked")
        if capt_m == 0 or capt_m == 1:
    ...
    def preview(self):
        global preview_data
        if capt_m == 0:
            preview_data = [self.frame1, self.frame2, self.current_mode, thr_sl_val, datetime.datetime.now(), self.temp, self.humidity]
        elif capt_m == 1:
            self.acc_counter = 1
            ...
            while self.acc_counter <= acc_period:
                ...
                self.frame_acc = ((self.frame_acc + self.frame2) / 2)
                time.sleep(.5)
                self.acc_counter += 1
            preview_data = [self.frame1, self.frame_acc, self.current_mode, thr_sl_val, datetime.datetime.now(), self.temp, self.humidity]
            self.shot_button.text = loc_dict['shot_t']
        elif capt_m == 2:
...
class ViewScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.dy = 0
        self.texture = Texture.create(size=(640, 480), colorfmt='bgr')
        self.texture2 = Texture.create(size=(128, 96), colorfmt='bgr')
        self.info_texture = Texture.create(size=(640, 480), colorfmt='bgr')
...
############self.frame2_thr = cv2.merge([(spot_color[2]*self.frame2_thr).astype(np.uint8), (spot_color[1]*self.frame2_thr).astype(np.uint8), (spot_color[0]*self.frame2_thr).astype(np.uint8)])
            ...
            self.texture2.blit_buffer(buf, colorfmt='bgr', bufferfmt='ubyte')
            self.miniature_canva.texture = self.texture2
...
class ImageScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.dx = 0
        self.dy = 0
        self.texture1 = Texture.create(size=(640, 480), colorfmt='bgr')
        self.texture2 = Texture.create(size=(128, 96), colorfmt='bgr')
    ...
    def render(self):
        ...
        self.im_view_canva.texture = self.texture1
        ...
        buf = buf2.tobytes()
        self.texture2.blit_buffer(buf, colorfmt='bgr', bufferfmt='ubyte')
        self.im_miniature_canva.texture = self.texture2
